...
import { CategoryFoodComponent } from './model';
...
export default function (injectedStore: any, injectedCache: any) {
  ...
  const table = 'category_food_component';
  ...
  async function insert({ datas, type }: any) {
    return new Promise(async (resolve, reject) => {
      ...
      const data = new CategoryFoodComponent(datas);
      try {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (!foods) {
          ...
          !foods && reject({ msg: 'No hay category_food_component de comida' });
  ...
  async function get(data: any) {
    return new Promise(async (resolve, reject) => {
      try {
        const { filter } = data;
        const theData = { type: 'getCategory_food_component', querys: filter };
        let food: CategoryFoodComponent = await cache.get(filter.id, table);
        if (!food) {
        ...
        return reject({ msg: 'maybe the category_food_component dont exist or is not active' });
      }
    });
  }
  ...
  async function update({ datas, id, type }: any) {
    return new Promise(async (resolve, reject) => {
      ...
      const data = Object.assign(new CategoryFoodComponent(datas), { id });

      try {
        const dataRespon: CategoryFoodComponent = await store.upsert(table, { data, type });
