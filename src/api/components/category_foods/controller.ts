...
import { CategoryFood } from './model';
...
export default function (injectedStore: any, injectedCache: any) {
  ...
  async function insert({ datas, type }: any) {
    return new Promise(async (resolve, reject) => {
      ...
      const data = new CategoryFood(datas);
      try {
  ...
  async function get(data: any) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let food: CategoryFood = await cache.get(filter.id, table);
        if (!food) {
  ...
  async function update({ datas, id, type }: any) {
    return new Promise(async (resolve, reject) => {
      ...
      const data = Object.assign(new CategoryFood(datas), { id });

      try {
        const dataRespon: CategoryFood = await store.upsert(table, { data, type });
